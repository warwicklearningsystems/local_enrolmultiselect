...
M.local_enrolmultiselect.user_selectors = [];
/**
 ...
 * @return bool
 */
M.local_enrolmultiselect.get_user_selector = function (name) {
 ...
 */
M.local_enrolmultiselect.init_user_selector = function (Y, name, hash, extrafields, lastsearch) {
    ...
    var user_selector = {
         ...
         */
        init : function() {
            $('#'+name).attr('data-selectorid', hash);
        ...
        send_query : function(forceresearch) {
            ...
            var iotrans = Y.io(M.cfg.wwwroot + '/local/enrolmultiselect/search.php', {
                method: 'POST',
        ...
        output_options : function(data) {
            ...
            if (!count) {
                var searchstr = (this.lastsearch != '') ? this.insert_type_into_str( this.insert_search_into_str(M.util.get_string('nomatchingusers', 'local_enrolmultiselect',{type:'Dep',search:this.lastsearch}), this.lastsearch), $('#'+name).parent().parent().find('.multiselect-label').html()) : M.util.get_string('none', 'moodle');
        ...
        output_group : function(groupname, users, selectedusers, processsingle) {
            ...
            if (count > 0) {
                optgroup.setAttribute('data-groupname', groupname);
         ...
         * @return string
         */
        insert_type_into_str : function(str, type) {
 ...
 */
M.local_enrolmultiselect.init_user_selector_options_tracker = function(Y) {
